...
module PetriPlotter = Plotter.Make (StringPTNet.G)
module ProductPlotter = Plotter.Make (StringPTNetProduct.G)
module FormulaPTPlotter = Plotter.Make (StringLtl.FormulaPTNet.G)
module UnfoldPlotter =
  Plotter.Make (StringProductUnfolder.Unfolder.OccurrenceNet.G)
...
let plot_unfold u =
  UnfoldPlotter.print_graph
    (module struct
      include (val StringProductUnfolder.Unfolder.OccurrenceNet.get_style u)
      let vertex_label v = String_product.label_of_unfold_node v
      let vertex_name v =
        Plotter.wrap_name @@ String_product.name_of_unfold_node v
      let vertex_attributes v =
        vertex_attributes v @ [ `Label (vertex_label v) ]
    end)
    (StringProductUnfolder.Unfolder.OccurrenceNet.get_graph u)
