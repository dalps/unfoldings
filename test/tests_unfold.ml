...
let%test "" =
  is_occurrence_sequence [ [ `T "t2" ]; [ `T "t1" ]; [ `T "t3" ] ] prod1
let%test "" =
  is_occurrence_sequence [ [ `T "t2" ]; [ `T "t3" ]; [ `T "t1" ] ] prod1
...
let%test "" =
  is_occurrence_sequence [ u1; t2; t4u2; t5; u3; u1; t4u2 ] prod2 = false

let%test "" =
  is_occurrence_sequence [ u1; t2; t4u2; t5; u3; t2; u1; t4u2 ] prod2
let%test "" =
  is_occurrence_sequence [ u1; t2; t4u2; t5; u3; t1; u1; t3u2 ] prod2
...
let%test "" =
  tword_equiv [ u1; t1; t3u2; t5; u3 ] [ u1; t1; t3u2; t4; u3 ] = false
let%test "" = tword_equiv [ u1; t1; t3u2; t5; u3 ] [ u1; t1; t3u2; t4 ] = false
let%test "" =
  tword_equiv [ u1; t1; t3u2; t5; u3 ] [ u1; t1; t3u2; t5; t4u2 ] = false
...
(* --- end is_freechoice --- *)
let%test "" =
  let r = Exe.test prod1 (d_compare sl_compare) [ [ `T "t1" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod1 (d_compare sl_compare) [ [ `T "t2" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod1 (d_compare sl_compare) [ [ `T "t3" ] ] 99 in
  r.res

(* --- end is_executable prod1 --- *)
let%test "" =
  let r = Exe.test prod2 (d_compare sl_compare) [ u3 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod2 (d_compare sl_compare) [ t4u2 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod2 (d_compare sl_compare) [ t5 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod2 (d_compare sl_compare) [ t3u2 ] 99 in
  r.res
...
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ b1 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ b2 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ b3 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ b4 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ a0a1 ] 99 in
  r.res
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ c ] 99 in
  r.res = false
let%test "" =
  let r = Exe.test prod3 (d_compare sl_compare) [ c; a0a1 ] 99 in
  r.res

(* --- end is_executable prod3 --- *)
let%test "" =
  let r =
    Exe.test prod4 (d_compare sl_compare) [ [ `Idle; `T "u1"; `T "v1" ] ] 99
  in
  r.res
let%test "" =
  let r =
    Exe.test prod4 (d_compare sl_compare) [ [ `Idle; `T "u1"; `Idle ] ] 99
  in
  r.res = false

let%test "" =
  let r =
    Exe.test prod4 (d_compare sl_compare) [ [ `T "t1"; `T "u1"; `Idle ] ] 99
  in
  r.res
...
let%test "" =
  let r =
    Exe.test prod5 (d_compare sl_compare)
      [ [ `Idle; `Idle; `T "f3"; `T "f4" ] ]
      99
  in
  r.res
...
let%test "" =
  let r =
    Exe.test prod5 sl_compare [ [ `T "i1"; `T "i2"; `T "i3"; `T "i4" ] ] 99
  in
  r.res
(* --- end is_executable prod5 --- *)

let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "A" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "B" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "C" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "D" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "E" ] ] 99 in
  r.res
let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "G" ] ] 99 in
  r.res
...
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "A" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "B" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "C" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "D" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "E" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "G" ] ] 99 in
  r.res

let _ = StringPTNetProduct.add_trans [ `T "F" ] Prod8.prod8

let%test "" =
  let r = Exe.test Prod8.prod8 (d_compare sl_compare) [ [ `T "F" ] ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test Prod8.prod8 (d_compare sl_compare) [ [ `T "F" ] ] 99 in
  r.res = false
...
let%test "" =
  let r =
    Exe.test Prod7.prod7 (d_compare sl_compare) [ [ `T "c"; `Idle ] ] 99
  in
  r.res
let%test "" =
  let r =
    Rep.test Prod7.prod7 (d_compare sl_compare) [ [ `T "c"; `Idle ] ] 99
  in
  r.res = false
let%test "" =
  let r =
    Rep.test Prod7.prod7 (d_compare sl_compare) [ [ `Idle; `T "a" ] ] 99
  in
  r.res
let%test "" =
  let r =
    Rep.test Prod7.prod7 (d_compare sl_compare) [ [ `Idle; `T "b" ] ] 99
  in
  r.res
...
let%test "" =
  let r = Rep.test prod6 (d_compare sl_compare) [ [ `T "r" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod6 (d_compare sl_compare) [ [ `T "c" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod6 (d_compare sl_compare) [ [ `T "a" ] ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod6 (d_compare sl_compare) [ [ `T "b" ] ] 99 in
  r.res = false

(* --- end is_infinitely_executable prod6 --- *)
let%test "" =
  let r = Rep.test prod1 (d_compare sl_compare) [ [ `T "t1" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod1 (d_compare sl_compare) [ [ `T "t2" ] ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod1 (d_compare sl_compare) [ [ `T "t3" ] ] 99 in
  r.res

(* --- end is_infinitely_executable prod1 --- *)
let%test "" =
  let r = Rep.test prod2 (d_compare sl_compare) [ u3 ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod2 (d_compare sl_compare) [ t4u2 ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod2 (d_compare sl_compare) [ t5 ] 99 in
  r.res
let%test "" =
  let r = Rep.test prod2 (d_compare sl_compare) [ t3u2 ] 99 in
  r.res
...
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ b1 ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ b2 ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ b3 ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ b4 ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ a0a1 ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ c ] 99 in
  r.res = false
let%test "" =
  let r = Rep.test prod3 (d_compare sl_compare) [ c; a0a1 ] 99 in
  r.res = false
(* --- end is_infinitely_executable prod3 --- *)

let%test "" =
  let r =
    Rep.test prod4 (d_compare sl_compare) [ [ `Idle; `T "u1"; `T "v1" ] ] 99
  in
  r.res = false

let%test "" =
  let r =
    Rep.test prod4 (d_compare sl_compare) [ [ `Idle; `T "u1"; `Idle ] ] 99
  in
  r.res = false

let%test "" =
  let r =
    Rep.test prod4 (d_compare sl_compare) [ [ `T "t1"; `T "u1"; `Idle ] ] 99
  in
  r.res = false
...
let%test "" =
  let r =
    Rep.test prod5 (d_compare sl_compare)
      [ [ `Idle; `Idle; `T "f3"; `T "f4" ] ]
      99
  in
  r.res = false
...
let%test "" =
  let r =
    Rep.test prod5 sl_compare [ [ `T "i1"; `T "i2"; `T "i3"; `T "i4" ] ] 99
  in
  r.res = false
...
let%test "" =
  TransSet.subset (transitions onet) (conv_tr @@ R.transitions rev_onet)
  ...
  TransSet.iter
    R.(
      fun t ->
        ...
        assert (
          PlaceSet.equal
            (preset_t rev_onet (`Rev (`E t)))
            (postset_t rev_onet (`E t)));
        assert (
          PlaceSet.equal
            (postset_t rev_onet (`Rev (`E t)))
            (preset_t rev_onet (`E t))))
    (transitions onet)
...
let%test "" =
  ReversibleEvent.compare (`Rev (`Rev (`E e1))) (`E e1) = Event.compare e1 e1
let%test "" =
  ReversibleEvent.compare (`Rev (`Rev (`E e1))) (`E e2) = Event.compare e1 e2
let%test "" = ReversibleEvent.compare (`Rev (`E e1)) (`E e1) = 1
let%test "" =
  ReversibleEvent.compare (`Rev (`E e1)) (`Rev (`E e2)) = Event.compare e1 e2
let%test "" =
  ReversibleEvent.compare (`Rev (`E e1)) (`Rev (`E e2)) = Event.compare e1 e2
let%test "" =
  PlaceSet.equal
    (preset_t rev_onet (`Rev (`Rev (`E e1))))
    (preset_t rev_onet (`E e1))
