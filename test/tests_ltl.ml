...
assert (is_error (test Prod7.prod7 (F (And (AP "t1", AP "t2")))));; (* prod7 can never be in both t1 and t2 at the same time *)
assert (is_error (test Prod7.prod7 (F (And (AP "u1", AP "u2")))));; (* prod7 can never be in both u2 and u1 at the same time *)
assert (is_error (test Prod7.prod7 (F (And (AP "u2", AP "u1")))));; (* and commutativity *)
assert (is_error (test Prod7.prod7 (G (And (AP "u1", AP "u2")))));; (* same as before *)

assert (is_ok (test Prod7.prod7 (F (Or (AP "u2", AP "u1")))));; (* prod7 will eventually be in either u2 or u1 *)
assert (is_ok (test Prod7.prod7 (G (Or (AP "u2", AP "u1")))));; (* prod7 is always in either u2 or u1 *)

assert (is_ok (test Prod7.prod7 (F (And (AP "u1", X (AP "u2"))))));; (* all infinite runs execute <ϵ,a> at least once *)
assert (is_error (test Prod7.prod7 (G (And (AP "u1", X (AP "u2"))))));; (* violated by all infinite runs as it requires prod7 to stay in u1 indefinitely *)
assert (is_ok (test Prod7.prod7 (F (If (AP "u1", X (AP "u2"))))));; (* more sensible *)
assert (is_ok (test Prod7.prod7 (F (Or (Not (AP "u1"), X (AP "u2"))))));; (* if equivalence *)
assert (is_ok (test Prod7.prod7 (F (G (If (AP "u1", X (AP "u2")))))));; (* all infinite runs alternate between u1 and u2 forever after (if ever) executing <c,ϵ> *)
  
assert (is_ok (test Prod7.prod7 (F (AP "t1"))));; (* obvious *)
assert (is_error (test Prod7.prod7 (F (AP "t2"))));; (* violated by the infinite run that never executes <c,ϵ> *)

assert (is_error (test Prod7.prod7 (F (And (AP "t2", X (AP "t1"))))));; (* prod7 cannot visit t1 from t2 *)
assert (is_ok (test Prod7.prod7 (Not (F (And (AP "t2", X (AP "t1")))))));; (* negation works *)

assert (is_error (test Prod7.prod7 (F (And (AP "t2", F (AP "t1"))))));; (* prod7 cannot visit t1 from t2 ever again *)
assert (is_ok (test Prod7.prod7 (F (And (AP "u2", X (AP "u1"))))));; (* prod7 can visit u2 from u1 *)
assert (is_ok (test Prod7.prod7 (F (And (AP "u2", F (AP "u1"))))));; (* prod7 can visit u2 from u1 *)

assert (is_ok (test Prod7.prod7 (G (F (AP "u1")))));; (* prod7 can visit u1 infinitely often *)
assert (is_error (test Prod7.prod7 (F (G (AP "u1")))));; (* prod7 cannot get stuck in u1 forever *)

assert (is_ok (test Prod7.prod7 (G (F (AP "u2")))));; (* prod7 can visit u1 infinitely often *)
assert (is_error (test Prod7.prod7 (F (G (AP "u2")))));; (* prod7 cannot get stuck in u2 forever *)

assert (is_ok (test Prod7.prod7 (G (F (AP "u1")))));; (* prod7 can visit t1 infinitely often *)
assert (is_error (test Prod7.prod7 (F (G (AP "t1")))));; (* prod7 CAN get stuck in t1 forever *)

assert (is_error (test Prod7.prod7 (G (F (AP "t2")))));; (* t2 can be visited only once *)
assert (is_error (test Prod7.prod7 (F (G (AP "t2")))));; (* prod7 doesn't get stuck forever in t2 in all runs *)
assert (is_error (test Prod7.prod7 (G (AP "t2"))));; (* prod7 doesn't start from t2 *)
assert (is_error (test Prod7.prod7 (G (G (AP "t2")))));; (* this is equal to 'G (AP "t2")' *)

assert (is_error (test Prod7.prod7 (F (And (AP "s4", AP "r3")))));; (* s4 is not a place of Prod7.prod7 *)
assert (is_ok (test Prod7.prod7 (F (Or (AP "s4", AP "u1")))));; (* s4 is not a place of Prod7.prod7, but u1 is *)

assert (is_error (test Prod2.prod2 (And (AP "s4", AP "s3"))));; (* Prod2.prod2 doesn't start from this marking *)
assert (is_ok (test Prod2.prod2 (F (And (AP "s4", AP "r3")))));; (* Prod2.prod2 will eventually reach this marking *)
assert (is_error (test Prod2.prod2 (F (And (AP "s4", AP "r1")))));; (* violated by the run that always executes <t5,ϵ> from the marking {s4,r3} *)
assert (is_error (test Prod2.prod2 (F (And (AP "s2", AP "s3")))));; (* but not this one - conflict *)
assert (is_error (test Prod2.prod2 (F (And (AP "s1", AP "s2")))));; (* neither this one - causality *)
assert (is_error (test Prod2.prod2 (F (And (AP "s1", F (AP "s2"))))));; (* violated by the run that always takes the s3 route *)
assert (is_error (test Prod2.prod2 (F (G (AP "s4")))));; (* no run stays forever in s4 *)

assert (is_error (test Prod2.prod2 (F (And (AP "s2", X (AP "s4"))))));; (* violated by the infinite run that always passes through s3 to go to s4 and never visit s2 *)
assert (is_error (test Prod2.prod2 (F (And (And (AP "s2", AP "r2"), X (AP "s4"))))));; (* same *)

assert (is_error (test Prod2.prod2 (F (AP "s3"))));; (* violated by the infinite run that always passes through s2 to go to s4 and never visit s3 *)
assert (is_error (test Prod2.prod2 (F (AP "s2"))));; (* violated by the infinite run that always passes through s3 to go to s4 and never visit s2 *)

assert (is_ok (test ~stutter:true Prod2.prod2 (G (F (AP "r1")))));;
assert (is_ok (test ~stutter:true Prod2.prod2 (G (F (AP "r2")))));;
assert (is_ok (test ~stutter:true Prod2.prod2 (G (F (AP "r3")))));;
assert (is_error (test Prod2.prod2 (G (F (AP "s2")))));;
assert (is_error (test Prod2.prod2 (G (F (AP "s3")))));;

assert (is_error (test Prod7.prod7 (G (If (AP "u1", X (AP "u1"))))));; (* violated by all runs *)
assert (is_error (test Prod7.prod7 (G (Or (Not (AP "u1"), X (AP "u1"))))));; (* violated by runs that execute <c,ϵ> *)

(* ##### FAILURES/UNCERTAIN ##### *)
(* these two assume <c,ϵ> will be executed eventually *)
assert (is_ok (test Prod2.prod2 (F (And (AP "s2", F (AP "s4"))))));; (* ??? should be violated by the infinite run that always passes through s3 to go to s4 and never visit s2 *)
assert (is_ok (test Prod7.prod7 (G (If (AP "u1", X (AP "u2"))))));; (* ??? should be violated by runs that execute <c,ϵ> *)
assert (is_ok (test Prod7.prod7 (G (If (AP "t1", X (AP "t2"))))));; (* ??? should be violated by runs that never execute <c,ϵ> *)
